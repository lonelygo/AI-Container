...
def draw_bounding_box_on_image_array(image,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  draw_bounding_box_on_image(image_pil, ymin, xmin, ymax, xmax, filename, color,
...
def draw_bounding_box_on_image(image,
  ...
  for display_str in display_str_list[::-1]:
    ...
    if display_str.split(':')[0] == 'id' :
      cropped_image.save(os.path.join('D:\\tensorflow-model\\research\\object_detection\\ssd_model_ctn\\CTN_crop\\id', filename))
    else:
      cropped_image.save(os.path.join('D:\\tensorflow-model\\research\\object_detection\\ssd_model_ctn\\CTN_crop\\size', filename))
...
def _visualize_boxes(image, boxes, classes, scores, category_index, filename, **kwargs):
  return visualize_boxes_and_labels_on_image_array(
      image, boxes, classes, scores, filename, category_index=category_index,  **kwargs)


def _visualize_boxes_and_masks(image, boxes, classes, scores, masks,
                               category_index, filename, **kwargs):
  return visualize_boxes_and_labels_on_image_array(
      image,
      boxes,
      classes,
...
def _visualize_boxes_and_masks_and_keypoints(
    image, boxes, classes, scores, masks, keypoints, category_index, filename, **kwargs):
  return visualize_boxes_and_labels_on_image_array(
      image,
      boxes,
      classes,
